...
function getPolynom(...argument) {
  ...
  const rever = arg.reverse();
  if (arg.length === 1) return () => rever[0];
  if (arg.length === 2) return (x) => rever[0] + x * rever[1];
  if (arg.length === 3) return (x) => rever[0] + x * rever[1] + x ** 2 * rever[2];
  return null;
}
